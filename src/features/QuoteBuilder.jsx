...
import { jsPDF } from 'jspdf'; // 這裡已經引入了，下面直接用
...
export default function QuoteBuilder({ templates, companyInfo, theme, db, appId, userId }) {
    ...
    // 這裡省略客戶管理的 useEffect 與相關函式 (與上一版相同，為節省篇幅不重複貼上，請確保保留)
    // 若您上一版已經更新了 QuoteBuilder 包含客戶管理功能，請保留那部分的代碼
    // 或是您可以再跟我說一聲，我給您包含客戶管理的完整版。
    // 為避免混亂，我這邊提供包含「下載修復」與「客戶管理」的完整版：

    // --- 客戶管理相關邏輯開始 ---
    const { onSnapshot, doc, setDoc, deleteDoc, updateDoc, getDocs } = require("firebase/firestore");
    ...
    const copyPublicLink = () => {
        ...
        navigator.clipboard.writeText(url)
            .then(() => toast.show("連結已複製！", "success"))
            .catch(() => prompt("請手動複製網址:", url));
    };
    ...
    const handleDownloadPDF = () => {
        ...
        if (!el) return toast.show("找不到報價單內容", "error");

        toast.show("正在生成 PDF...", "info");
        
        html2canvas(el, { 
            scale: 2, // 提高清晰度
            useCORS: true, // 允許跨域圖片 (Logo)
            logging: false
        }).then(canvas => {
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-8">
                ...
                <div className={`p-8 rounded-4xl border ${panelClass}`}>
                    ...
                    <div className="pt-6 border-t border-gray-500/20 grid grid-cols-3 gap-4">
                        <input value={quoteData.versionNote} onChange={e=>setQuoteData({...quoteData, versionNote: e.target.value})} className={`w-full p-2 text-sm rounded-xl border text-center ${inputClass}`} placeholder="版本註記 (例: V2)" />
                        ...
                        <input type="number" step="0.01" value={quoteData.taxRate} onChange={e=>setQuoteData({...quoteData, taxRate: parseFloat(e.target.value)})} className={`w-full p-2 text-sm rounded-xl border text-center ${inputClass}`} placeholder="稅率" />
                    </div>
                </div>
                ...
                <div className={`p-8 rounded-4xl border ${panelClass}`}>
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <div className="flex gap-2">
                            <button onClick={addTextRow} className="text-sm bg-gray-500/10 text-gray-500 px-3 py-2 rounded-xl font-bold hover:bg-gray-500 hover:text-white transition">+ 文字</button>
                            <button onClick={addManual} className="text-sm bg-dc-orange/10 text-dc-orange px-4 py-2 rounded-xl font-bold hover:bg-dc-orange hover:text-white transition">+ 自訂</button>
                        </div>
                    </div>
            ...
            <div className="space-y-8">
                ...
                <div className={`p-8 rounded-4xl border ${panelClass}`}>
                    ...
                    <div className="grid gap-3 max-h-[400px] overflow-auto pr-1 custom-scrollbar">
                        {templates.map(t => (
